...
public class StamboomFXController extends StamboomController implements Initializable {
    ...
    @FXML AnchorPane apStamboom;
    @FXML TreeView tvStamboom;
    ...
    private void initComboboxes() {
        ...
        getAdministratie().addPersoon(Geslacht.MAN, new String[] {"Opa", }, "Janssen", "", c, "Eindhoven", null);
        getAdministratie().addPersoon(Geslacht.VROUW, new String[] {"Oma", }, "Iets", "van", c, "Eindhoven", null);
        getAdministratie().addPersoon(Geslacht.MAN, new String[] {"Vader", }, "Janssen", "", c, "Eindhoven", null);
        getAdministratie().addPersoon(Geslacht.VROUW, new String[] {"Moeder", }, "IetsAnders", "van", c, "Eindhoven", null);
        getAdministratie().addPersoon(Geslacht.MAN, new String[] {"Kind", }, "Janssen", "", c, "Eindhoven", null);
        
        date = new Date(2001, 1, 1);
        c.setTime(date);
        getAdministratie().addHuwelijk(getAdministratie().getPersoon(1), getAdministratie().getPersoon(2), c);
        
        date = new Date(2002, 1, 1);
        c.setTime(date);
        getAdministratie().addHuwelijk(getAdministratie().getPersoon(3), getAdministratie().getPersoon(4), c);
        
        getAdministratie().setOuders(getAdministratie().getPersoon(3), getAdministratie().getGezin(1));
        getAdministratie().setOuders(getAdministratie().getPersoon(5), getAdministratie().getGezin(2));
    }
    ...
    public void showStamboom(Event evt) {
        ...
        if (persoon != null)
        {
            ...
            //NEW:
            TreeItem<String> root = new TreeItem<String>(persoon.getNaam());
            root.setExpanded(true);
            
            apStamboom.getChildren().add(new TreeView(root));
        }
    }
    ...
    private void clearTabStamboom()
    {
        apStamboom.getChildren().removeAll();
    }
    
    private void setupTabStamboom()
    {
        TreeItem<String> root = new TreeItem<String>("Personen");
        
        for (Persoon p : getAdministratie().getPersonen())
        {
            if (p.getAlsOuderBetrokkenIn().size() == 0)
            {
                TreeItem ti = getTreeItem(p);
                if (ti != null)
                {
    ...
    private TreeItem<String> getTreeItem(Persoon p)
    {
        TreeItem<String> ti = new TreeItem<String>(p.getNaam());
        if (p.getOuderlijkGezin() != null)
        {
            if (p.getOuderlijkGezin().getOuder1() != null)
            {
                ti.getChildren().add(getTreeItem(p.getOuderlijkGezin().getOuder1()));
            }
            if (p.getOuderlijkGezin().getOuder2() != null)
            {
                ti.getChildren().add(getTreeItem(p.getOuderlijkGezin().getOuder2()));
            }
        } 
        return ti;
    }
